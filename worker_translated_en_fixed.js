...
export default {
    async fetch(request, env, ctx) {
        ...
        const 加密秘钥 = env.KEY || 'Do not change this default key. If you need to modify it, set the KEY environment variable.';
        ...
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            if (访问路径 === 加密秘钥 && 加密秘钥 !== 'Do not change this default key. If you need to modify it, set the KEY environment variable.') {// Quick subscription
                const params = new URLSearchParams(url.search);
                params.set('token', await MD5MD5(host + userID));
                return new Response('Redirecting...', { status: 302, headers: { 'Location': `/sub?${params.toString()}` } });
            } else if (访问路径 === 'login') {// Handle the login page and login requests
                ...
                if (request.method === 'POST') {
                    ...
                    if (输入密码 === 管理员密码) {
                        // Password is correct; set the cookie and return a success flag
            ...
            } else if (访问路径 === 'admin' || 访问路径.startsWith('admin/')) {// Serve the admin page after validating the cookie
                const cookies = request.headers.get('Cookie') || '';
                const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
                // No cookie or invalid cookie; redirect to /login
                if (!authCookie || authCookie !== await MD5MD5(UA + 加密秘钥 + 管理员密码)) return new Response('Redirecting...', { status: 302, headers: { 'Location': '/login' } });
                if (访问路径 === 'admin/log.json') {// Read log contents
                    const 读取日志内容 = await env.KV.get('log.json') || '[]';
                    return new Response(读取日志内容, { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                } else if (区分大小写访问路径 === 'admin/getCloudflareUsage') {// Query request volume
                    try {
                        const Usage_JSON = await getCloudflareUsage(url.searchParams.get('Email'), url.searchParams.get('GlobalAPIKey'), url.searchParams.get('AccountID'), url.searchParams.get('APIToken'));
                        return new Response(JSON.stringify(Usage_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json' } });
                    } catch (err) {
                        const errorResponse = { msg: 'Failed to query request volume. Reason: ' + err.message, error: err.message };
                        return new Response(JSON.stringify(errorResponse, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
                } else if (区分大小写访问路径 === 'admin/getADDAPI') {// Validate the preferred-selection API
                    if (url.searchParams.get('url')) {
                        ...
                        try {
                            ...
                            const errorResponse = { msg: 'Failed to validate the preferred-selection API. Reason: ' + err.message, error: err.message };
                            return new Response(JSON.stringify(errorResponse, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        }
                    }
                    return new Response(JSON.stringify({ success: false, data: [] }, null, 2), { status: 403, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                } else if (访问路径 === 'admin/check') {// SOCKS5 proxy check
                    let 检测代理响应;
                ...
                if (访问路径 === 'admin/init') {// Reset configuration to defaults
                    try {
                        ...
                        const errorResponse = { msg: 'Failed to reset the configuration. Reason: ' + err.message, error: err.message };
                        return new Response(JSON.stringify(errorResponse, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
                } else if (request.method === 'POST') {// Handle KV operations (POST requests)
                    if (访问路径 === 'admin/config.json') { // Save config.json configuration
                        try {
                            ...
                            return new Response(JSON.stringify({ success: true, message: 'Configuration has been saved.' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        } catch (error) {
                    ...
                    } else if (访问路径 === 'admin/cf.json') { // Save cf.json configuration
                        try {
                            const newConfig = await request.json();
                    ...
                    } else if (访问路径 === 'admin/tg.json') { // Save tg.json configuration
                        try {
                            const newConfig = await request.json();
                            if (newConfig.init && newConfig.init === true) {
                    ...
                    } else if (区分大小写访问路径 === 'admin/ADD.txt') { // Save custom preferred IPs
                        try {
                            const customIPs = await request.text();
                            await env.KV.put('ADD.txt', customIPs);// Save to KV
                            ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Custom_IPs', config_JSON));
                            return new Response(JSON.stringify({ success: true, message: 'Custom IPs have been saved.' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        } catch (error) {
                ...
                } else if (访问路径 === 'admin/config.json') {// Handle /admin/config.json request and return JSON
                    return new Response(JSON.stringify(config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json' } });
                } else if (区分大小写访问路径 === 'admin/ADD.txt') {// Handle /admin/ADD.txt request and return local preferred IPs
                    let 本地优选IP = await env.KV.get('ADD.txt') || 'null';
                    if (本地优选IP == 'null') 本地优选IP = (await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口))[1];
                    return new Response(本地优选IP, { status: 200, headers: { 'Content-Type': 'text/plain;charset=utf-8', 'asn': request.cf.asn } });
                } else if (访问路径 === 'admin/cf.json') {// CF configuration file
                ...
                return 响应;
            } else if (访问路径 === 'sub') {// Handle subscription requests
                ...
                if (url.searchParams.get('token') === 订阅TOKEN) {
                    ...
                    const expire = 4102329600;// Expiry date: 2099-12-31
                    const now = Date.now();
                    ...
                    if (订阅类型 === 'mixed') {
                        ...
                        if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // Generate subscription locally
                            ...
                            优选订阅生成器HOST = 优选订阅生成器HOST && !/^https?:\/\//i.test(优选订阅生成器HOST) ? `https://${优选订阅生成器HOST}` : 优选订阅生成器HOST; // Ensure the generator host has a scheme (prepend https:// if missing)
                            ...
                            try {
                                ...
                                if (!response.ok) return new Response('Preferred subscription generator error: ' + response.statusText, { status: response.status });
                                const 优选订阅生成器返回订阅内容 = atob(await response.text());
                                const 订阅行列表 = 优选订阅生成器返回订阅内容.includes('\r\n') ? 优选订阅生成器返回订阅内容.split('\r\n') : 优选订阅生成器返回订阅内容.split('\n');
                                for (const 行内容 of 订阅行列表) {
                                    if (!行内容.trim()) continue; // Skip empty lines
                                    if (行内容.includes('00000000-0000-4000-8000-000000000000') && 行内容.includes('example.com')) { // This is a preferred-IP line; extract domain:port#remark
                                        const 地址匹配 = 行内容.match(/:\/\/[^@]+@([^?]+)/);
                                        if (地址匹配) {
                                            let 地址端口 = 地址匹配[1], 备注 = ''; // Domain:port or IP:port
                        ...
                        订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
                            // Unified regex: match domain/IPv4/IPv6 + optional port + optional remark
                            // Examples:
                            ...
                            if (match) {
                                节点地址 = match[1];  // IP address or domain (may include brackets)
                                节点端口 = match[2] || "443";  // Port (default: 443)
                                节点备注 = match[3] || 节点地址;  // Remark (defaults to the address itself)
                            } else {
                                // Non-standard format: skip and return null
                                console.warn(`[Subscription content] Ignored non-standard IP format: ${原始地址}`);
                                return null;
                            }
            ...
            } else if (访问路径 === 'locations') {// Proxy locations list
                const cookies = request.headers.get('Cookie') || '';
                const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
                if (authCookie && authCookie == await MD5MD5(UA + 加密秘钥 + 管理员密码)) return fetch(new Request('https://speed.cloudflare.com/locations', { headers: { 'Referer': 'https://speed.cloudflare.com/' } }));
            }
        } else if (管理员密码) {// WebSocket proxy
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
    console.log(`[TCP Forward] Target: ${host}:${portNum} | Proxy IP: ${反代IP} | Proxy fallback: ${启用反代兜底 ? 'Yes' : 'No'} | Proxy type: ${启用SOCKS5反代 || 'proxyip'} | Global: ${启用SOCKS5全局反代 ? 'Yes' : 'No'}`);
    ...
    async function connectDirect(address, port, data, 所有反代数组 = null, 反代兜底 = true) {
        ...
        if (所有反代数组 && 所有反代数组.length > 0) {
            for (let i = 0; i < 所有反代数组.length; i++) {
                ...
                try {
                    console.log(`[Proxy connection] Attempting to connect to: ${反代地址}:${反代端口} (Index: ${反代数组索引})`);
                    remoteSock = connect({ hostname: 反代地址, port: 反代端口 });
                    // Wait for the TCP connection to be fully established (1-second timeout)
                    await Promise.race([
                        remoteSock.opened,
                    ...
                    console.log(`[Proxy connection] Successfully connected to: ${反代地址}:${反代端口}`);
                    缓存反代数组索引 = 反代数组索引;
                    return remoteSock;
                } catch (err) {
                    console.log(`[Proxy connection] Connection failed: ${反代地址}:${反代端口}, Error: ${err.message}`);
    ...
    async function connecttoPry() {
        ...
        if (启用SOCKS5反代 === 'socks5') {
            ...
            console.log(`[Proxy connection] Proxying to: ${host}:${portNum}`);
    ...
    if (启用SOCKS5反代 && (启用SOCKS5全局反代 || 验证SOCKS5白名单(host))) {
        console.log(`[TCP Forward] SOCKS5/HTTP global proxy enabled`);
        try {
            await connecttoPry();
        } catch (err) {
            throw err;
...
function 掩码敏感信息(文本, 前缀长度 = 3, 后缀长度 = 2) {
    ...
    if (文本.length <= 前缀长度 + 后缀长度) return 文本; // If the length is too short, return as-is
...
async function 读取config_JSON(env, hostname, userID, path, 重置配置 = false) {
    // const host = randomizeWildcard(hostname);
    const host = hostname;
    ...
    const 默认配置JSON = {
        ...
        随机路径: false,
        优选订阅生成: {
            local: true, // true: use locally sourced preferred addresses; false: use the subscription generator
            本地IP库: {
                随机IP: true, // When RandomIP is true: number of random IPs to generate; otherwise use ADD.txt from KV
                随机数量: 16,
                指定端口: -1,
            },
            SUB: null,
            SUBNAME: "edge" + "tunnel",
            SUBUpdateTime: 6, // Subscription refresh interval (hours)
...
async function getCloudflareUsage(Email, GlobalAPIKey, AccountID, APIToken) {
    ...
    try {
        ...
        if (!AccountID) {
            ...
            if (!r.ok) throw new Error(`Failed to retrieve accounts: ${r.status}`);
    ...
    if (!缓存反代IP || !缓存反代解析数组 || 缓存反代IP !== proxyIP) {
        ...
        async function DoH查询(域名, 记录类型) {
            try {
                ...
                console.error(`DoH query failed (${记录类型}):`, error);
                return [];
            }
        }
        ...
        if (proxyIP.includes('.william')) {
            ...
            // Check whether it is a domain name (not an IP address)
        ...
        console.log(`[Proxy resolution] Random seed: ${随机种子}\nTarget site: ${目标根域名}`)
        const 洗牌后 = [...排序后数组].sort(() => (随机种子 = (随机种子 * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff - 0.5);
        缓存反代解析数组 = 洗牌后.slice(0, 8);
        console.log(`[Proxy resolution] Completed. Total: ${缓存反代解析数组.length} entries\n${缓存反代解析数组.map(([ip, port], index) => `${index + 1}. ${ip}:${port}`).join('\n')}`);
        缓存反代IP = proxyIP;
    } else console.log(`[Proxy resolution] Loaded from cache. Total: ${缓存反代解析数组.length} entries\n${缓存反代解析数组.map(([ip, port], index) => `${index + 1}. ${ip}:${port}`).join('\n')}`);
    return 缓存反代解析数组;
}
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数) {
    ...
    try {
        ...
        if (!tcpSocket) return { success: false, error: 'Unable to connect to the proxy server', proxy: 代理协议 + "://" + 完整代理参数, responseTime: Date.now() - startTime };
        try {
            ...
            try { await tcpSocket.close(); } catch (e) { console.log('Error while closing connection:', e); }
